...
class PlanckGrid():
    def __init__(self, size:int) -> None:
        ...
        # self.tail_pointers = [Pointer('N') for _ in range(size-1)]
    ...
    def add_tail_pos(self):
        ...
        if tail not in self.tail_positions:
            ...
            print (f'tail position {tail} already visited: {self.tail_positions}')
    ...
    def move_node(self, head, tail):
        ...
        if abs(diff_x) == 1 and abs(diff_y) == 1:
            ...
            tail.x += math.copysign(1, diff_x) 
        elif diff_x == 0 and 2 > abs(diff_y) > 1:
            # take step y-direction
            tail.y += math.copysign(1, diff_y) 
        elif 2 > abs(diff_x) > 1 and 2 > abs(diff_y) > 1:
            # take diagonal step
            tail.x += math.copysign(1, diff_x) 
            tail.y += math.copysign(1, diff_y) 
        else:
            raise Exception('Something in the logic isn\'t right')
...
def main():
    with open("input_day9.txt", "r") as input:
